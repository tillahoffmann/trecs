...
@pytest.fixture
def example_db_path(tmp_path: Path) -> Path:
...
@pytest.fixture
def example_conn(example_db_path: Path) -> Generator[sqlite3.Connection, None, None]:
    # Connect to it and yield the connection.
    with closing(sqlite3.connect(example_db_path)) as conn:
        yield conn
